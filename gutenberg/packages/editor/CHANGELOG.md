...
-   Collaboration: Replace the inline pending-edit card with a prototype conflict review flow. A conflicted block's edit UI is replaced in place by a recovery-style card ("Review conflict" above an add/remove highlighted preview), making the block read-only until reviewed. The card opens a dialog that compares "Your version" with the "Current version", each rendered as read-only blocks diffed against the shared base with the revisions diff system (block-level status markers plus inline ins/del highlighting), restores either into the merged result, edited in its own small block editor with text formatting controls, and on Accept writes that result into the block and clears the set-aside items. The merged editor stays free of diff highlighting on purpose: the inline marks are rich-text formats and would serialize into the accepted result. The compared texts are pre-set placeholder content while the UI design settles.
-   Collaboration: Add a table-shaped merge review to the conflict prototype. A conflicted table block's in-place card previews the changed cells as a compact table, and its dialog renders both versions as tables highlighting each side's own changes against the shared base and seeds the merged result with a computed cell-level merge: clean changes from both sides applied, and a cell both sides changed differently holding the current version's value, settled by editing the merged table directly. Accept writes the merged table's head and body attributes back into the block. The compared grids are pre-set placeholder content while the UI design settles.
-   Collaboration: Add a section-shaped merge review to the conflict prototype, for conflicts with no single-block answer, like a paragraph split on one side and edited on the other. When the conflicted block is a group, or sits inside one, the in-place card presents the conflict as the section's, and its dialog renders each version's blocks with the revisions diff system against the shared base, so a split reads as a modified paragraph plus an added one. The merged result is a multi-block editor with its structure unlocked, so resolving can keep, drop, or reshape blocks; Accept replaces the section's inner blocks with the merged result. Conflict items landing anywhere inside one group present as a single conflict group: the section's first conflicted block renders the card and dialog, its other conflicted blocks keep their normal edit UI, and Accept resolves the whole group (the principled grouping signal is the review items' unitId, the engines' atomic unit; the section gathering stands in for it until intent-log stamps txn ids onto captured batches). The compared versions are pre-set placeholder content while the UI design settles.
-   Collaboration: Add a prototype review flow for blocks held back by the security filter. A held block's edit UI is replaced in place by a recovery-style card ("This block requires elevated permissions." above the held markup as inert text), with a "Review changes" action for users allowed to publish unfiltered HTML. The action opens a dialog showing the held markup as the revisions system's line-numbered code diff, kept as inert text: every line reads as added for a new proposal, removed and added lines interleave in one unified view for an update. The dialog offers Approve, Remove block, and plain-text editing of the proposed markup with the diff recomputing live. The card triggers on real parked requires-approval review items, which no longer render the conflict card. Approve resolves them through the engines' restore lane, so the real held markup lands for every collaborator; whether the dialog shows the new-block or the update layout follows from whether the held block kept prior content (judged from the content attribute, or the inner markup of raw-content blocks like Custom HTML). The dialog's preview contents are pre-set placeholder scenarios while the UI design settles.
...
-   Extract the revisions block diff's visual layer (the diff styles, the removed-block SVG filter, the accessible diff descriptions, and the `editor.BlockListBlock` filter stamping diff status classes and labels) from the revisions canvas into `post-revisions-preview/block-diff-view.js`, so surfaces outside the revisions screen (the collaboration review dialogs) can render diffed blocks without pulling in the canvas.
