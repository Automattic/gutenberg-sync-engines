...
/**
 * The visual layer for rendering blocks produced by `diffRevisionContent`:
 * the diff CSS, the removed-block SVG filter, the accessible descriptions,
 * and the `editor.BlockListBlock` filter that stamps diff status classes
 * and labels onto blocks. Shared by the revisions canvas and the
 * collaboration review dialogs, which render diffed blocks outside the
 * revisions screen.
 */
...
export const REVISION_REMOVED_FILTER_SVG = `
<svg
...
/**
 * CSS for revision diff indicators. The revisions canvas injects it into
 * its iframe; other hosts (the collaboration merge dialog) render it in a
 * style tag, together with the SVG filter above, since the removed styles
 * reference the filter by id in the same document.
 * Uses color-mix() to blend diff colors with currentColor for better integration.
 */
export const REVISION_DIFF_STYLES = `
	.is-revision-added {
 ...
 * reference via `aria-describedby`. They must be rendered in the same
 * document as the marks because `aria-describedby` cannot reference an
 * element across a document/iframe boundary: the revisions canvas renders
 * them inside its iframe, the merge dialog inside the modal. This is more
 * reliable than `title`, which some screen readers ignore in
 * low-verbosity modes.
 */
