...
-   The review SPI gains an optional merge-view surface. `SyncReviewItem` adds `targetField` (the conflicted field's name, completing the author/block/field grouping key), `supportsMergeView` (whether the engine can serve the item through the merge dialog), and `groupSummary` (the whole changeset's lost content as one string, stamped identically on every member, so UIs show a parked burst once instead of joining per-keystroke summaries), and `SyncReviewSource` adds two optional methods the manager delegates when present: `describeReviewGroup` (the base, intended, and current texts of one group of review items, as a `SyncReviewGroupDescription`) and `resolveReviewGroup` (settle a whole group with one decision, optionally authoring a hand-merged result first).
