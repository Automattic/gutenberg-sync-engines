<?php
/**
 * REST API: WP_Sync_Engines_REST_Autosaves_Controller class
 *
 ...
 * This overrides the core WP_REST_Autosaves_Controller so draft autosaves
 * become revisions while several people edit one post.
 *
 * @see WP_REST_Autosaves_Controller
 */
class WP_Sync_Engines_REST_Autosaves_Controller extends WP_REST_Autosaves_Controller {
	 ...
	 */
	private $sync_engines_parent_controller;
	...
	public function __construct( $parent_post_type ) {
		...
		$this->sync_engines_parent_controller = $parent_controller;
	}
	...
	public function create_item( $request ) {
		// The plugin selects this controller only when collaboration is
		// enabled. Preserve Core behavior if it is registered explicitly or
		...
		if ( ! defined( 'WP_RUN_CORE_TESTS' ) && ! defined( 'DOING_AUTOSAVE' ) ) {
			define( 'DOING_AUTOSAVE', true ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedConstantFound -- Core's own constant.
		}

		$prepared_post     = $this->sync_engines_parent_controller->prepare_item_for_database( $request );
		$prepared_post->ID = $post->ID;
		$post_data         = (array) $prepared_post;
		$meta              = (array) $request->get_param( 'meta' );

		/*
		 * Regular draft autosaves must not update the parent post directly while
		 * collaborating. All peers share one editing session, so their
		 ...
		 * shared document and duplicated.
		 *
	 ...
	 * That baseline is wrong under collaboration. The parent draft is intentionally never
	 ...
	 * than the parent (the collaboration case), falling back to the parent when no revision
	 ...
	 * itself diffs. Non-revisioned meta is excluded.
	 *
	 * @since n.e.x.t
	 *
	 ...
	 *
	 * Under collaboration the parent draft is not updated with an autosave, so its latest content
	...
	private function get_autosave_comparison_baseline( $post ) {
		...
		// as newer than it. That is the desired outcome: under collaboration the
