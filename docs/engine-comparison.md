...
| Synced entity properties | Whitelist: the framework's scalar set (title, excerpt, slug, status, comment_status, ping_status, format, sticky, author, featured_media, date, template) plus attached taxonomies (whole term-ID arrays by rest_base), all as per-name registers; post meta still rides saves | Whatever the sync config maps into the CRDT (the full framework set, including per-key post meta and taxonomies) | Content only (**no title sync yet**) |
