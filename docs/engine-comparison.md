...
| --- | --- | --- | --- |
| Conflict handling | Transform on the server; genuine conflicts park in the editor's review panel (escalation notice, marker chip, durable resolutions — e2e-verified). The merge dialog covers a whole conflicted field in one decision: the author's intended text is replayed from the retained log (base plus their own rows, in their ORIGINAL authored form), shown word-diffed against the current text with the author's changeset factored out (a burst's already-merged first keystroke is part of the changeset, not of "current"), and resolved by keep-current (which discards the whole changeset, merged fragment included), restore-mine (a whole-field rewrite plus its formats), or a hand-merged result — e2e-verified | Silent CRDT auto-merge, but ON THE SERVER — outcomes observable, still no review lane (conflict DETECTION is the undesigned prerequisite; the merge dialog therefore has nothing to open on) | Three-way merge on the server; genuine conflicts PARK as durable `proposal-parked` rows and present in the same review panel (restore re-proposes under the reviewer; dismiss resolves; retention survives compaction — e2e-verified). The merge dialog serves the author's own parked rows (stored proposed blocks vs the live canonical block; a hand-merge re-proposes through the restore overlay), lost property values, and contested blocks — the one three-pane-guaranteed case, since the base block's form is now recorded when the contest arises — e2e-verified |
