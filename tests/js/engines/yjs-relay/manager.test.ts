...
const PROVIDERS_FILTER = 'sync.providers';
const PROVIDERS_HOOK = 'test/yjs-relay-manager';
...
describe( 'SyncManager', () => {
	...
	beforeEach( () => {
		...
		// getProviderCreators() short-circuits to [] unless collaboration is
		// enabled; the tests drive the manager as if it were.
		...
		addFilter( PROVIDERS_FILTER, PROVIDERS_HOOK, () => [
			mockProviderCreator,
		] );
	...
	afterEach( () => {
		removeFilter( PROVIDERS_FILTER, PROVIDERS_HOOK );
		resetProviderCreatorsForTesting();
	...
	describe( 'load', () => {
		...
		it( 'creates providers for the entity', async () => {
			...
			// Providers receive an engine session codec, never the Y.Doc or
			...
			expect( mockProviderCreator ).toHaveBeenCalledWith( {
				...
				session: expect.objectContaining( {
					...
					onLocalUpdate: expect.any( Function ),
					receiveUpdate: expect.any( Function ),
				} ),
			} );
		} );

		it( 'does not load entity when no providers are available', async () => {
			// Collaboration disabled → getProviderCreators() yields none, so
			// syncing is effectively off.
			delete window._wpCollaborationEnabled;
			resetProviderCreatorsForTesting();
	...
	describe( 'update', () => {
		it( 'updates CRDT document with local changes', async () => {
			// Providers no longer receive the Y.Doc; capture it via the sync
			// config, which receives it during load.
