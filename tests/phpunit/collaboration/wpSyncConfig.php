<?php
/**
 * Tests for the WP_Sync_Engines_Config class.
 *
...
class Tests_Collaboration_WpSyncConfig extends WP_UnitTestCase {
	...
	public function test_parse_room_for_single_entity() {
		$this->assertSame(
			...
			WP_Sync_Engines_Config::parse_room( 'postType/post:100' )
		);
	}
	...
	public function test_parse_room_for_collection() {
		$this->assertSame(
			...
			WP_Sync_Engines_Config::parse_room( 'taxonomy/category' )
		);
	}
	...
	public function test_parse_room_rejects_invalid_room_identifiers( string $room ) {
		$this->assertNull( WP_Sync_Engines_Config::parse_room( $room ) );
	}
	...
	public function test_can_user_sync_single_post_entity() {
		...
		$this->assertTrue( WP_Sync_Engines_Config::can_user_sync_entity_type( 'postType', 'post', (string) self::$post_id ) );
	}

	public function test_can_user_sync_single_page_entity() {
		wp_set_current_user( self::$editor_id );

		$this->assertTrue( WP_Sync_Engines_Config::can_user_sync_entity_type( 'postType', 'page', (string) self::$page_id ) );
	}

	public function test_can_user_sync_single_post_entity_requires_edit_permission() {
		wp_set_current_user( self::$subscriber_id );

		$this->assertFalse( WP_Sync_Engines_Config::can_user_sync_entity_type( 'postType', 'post', (string) self::$post_id ) );
	}
	...
	public function test_can_user_sync_single_post_entity_rejects_invalid_ids( string $entity_name, string $object_id ) {
		...
		$this->assertFalse( WP_Sync_Engines_Config::can_user_sync_entity_type( 'postType', $entity_name, $object_id ) );
	}
	...
	public function test_can_user_sync_single_post_entity_rejects_wrong_post_type() {
		...
		$this->assertFalse( WP_Sync_Engines_Config::can_user_sync_entity_type( 'postType', 'page', (string) self::$post_id ) );
	}

	public function test_can_user_sync_post_type_collection() {
		wp_set_current_user( self::$editor_id );

		$this->assertTrue( WP_Sync_Engines_Config::can_user_sync_entity_type( 'postType', 'post', null ) );
	}

	public function test_can_user_sync_page_collection() {
		wp_set_current_user( self::$editor_id );

		$this->assertTrue( WP_Sync_Engines_Config::can_user_sync_entity_type( 'postType', 'page', null ) );
	}

	public function test_can_user_sync_post_type_collection_requires_collection_capability() {
		wp_set_current_user( self::$subscriber_id );

		$this->assertFalse( WP_Sync_Engines_Config::can_user_sync_entity_type( 'postType', 'post', null ) );
	}

	public function test_can_user_sync_single_taxonomy_term() {
		wp_set_current_user( self::$editor_id );

		$this->assertTrue( WP_Sync_Engines_Config::can_user_sync_entity_type( 'taxonomy', 'category', (string) self::$category_id ) );
	}
	...
	public function test_can_user_sync_single_taxonomy_term_rejects_invalid_ids( string $taxonomy, string $object_id ) {
		...
		$this->assertFalse( WP_Sync_Engines_Config::can_user_sync_entity_type( 'taxonomy', $taxonomy, $object_id ) );
	}
	...
	public function test_can_user_sync_single_taxonomy_term_rejects_wrong_taxonomy() {
		...
		$this->assertFalse( WP_Sync_Engines_Config::can_user_sync_entity_type( 'taxonomy', 'category', (string) self::$tag_id ) );
	}

	public function test_can_user_sync_single_comment_entity() {
		wp_set_current_user( self::$editor_id );

		$this->assertTrue( WP_Sync_Engines_Config::can_user_sync_entity_type( 'root', 'comment', (string) self::$comment_id ) );
	}
	...
	public function test_can_user_sync_allowed_collections( string $entity_kind, string $entity_name ) {
		...
		$this->assertTrue( WP_Sync_Engines_Config::can_user_sync_entity_type( $entity_kind, $entity_name, null ) );
	}
	...
	public function test_can_user_sync_rejects_unknown_collection_entity_type() {
		...
		$this->assertFalse( WP_Sync_Engines_Config::can_user_sync_entity_type( 'unknown', 'entity', null ) );
	}

	public function test_can_user_sync_rejects_object_id_for_unsupported_single_entity_type() {
		wp_set_current_user( self::$editor_id );

		$this->assertFalse( WP_Sync_Engines_Config::can_user_sync_entity_type( 'root', 'site', '123' ) );
	}
}
