...
class Tests_Collaboration_RestAutosavesController extends WP_UnitTestCase {
	...
	// A meta key that is not revisioned, so changing it alone is not a new revision.
	private const UNREVISIONED_META_KEY = '_sync_engines_unrevisioned';
	...
	public function test_does_not_override_autosaves_controller_when_collaboration_is_disabled() {
		...
		$args = gutenberg_sync_engines_override_autosaves_rest_controller( array() );
	...
	public function test_does_not_override_explicit_autosaves_controller() {
		$args = gutenberg_sync_engines_override_autosaves_rest_controller(
	...
	/**
	 * Handles an autosave request with the plugin's controller.
	 *
	...
	private function dispatch_autosave( int $post_id, string $title, string $content, array $meta = array() ): WP_REST_Response {
		...
		/*
		 * Individual tests toggle collaboration after post types have been
		 * registered and their autosave controller classes selected. Changing
		 * the setting does not update that selection, so REST dispatch may
		 * exercise the wrong implementation for the test state. Invoke the
		 * plugin's controller directly and normalize its response as the REST
		 * server would.
		 */
		$controller = new WP_Sync_Engines_REST_Autosaves_Controller( 'post' );
	...
	private function assert_custom_controller_uses_core_autosave_behavior(): void {
		...
		$this->assertFalse(
			...
			'Core should update the author draft directly instead of creating a collaboration revision.'
		);
	}
	...
	public function test_draft_autosave_does_not_create_revision_when_content_is_unchanged() {
		...
		$response = $this->dispatch_autosave(
			...
			$content,
			array(
				self::UNREVISIONED_META_KEY => 'changed-crdt-doc',
			)
		);
